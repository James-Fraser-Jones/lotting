...
var author$project$Main$CellSelected = F2(
	function (a, b) {
		return {$: 'CellSelected', a: a, b: b};
	});
...
var author$project$Main$createCell = F4(
	function (point, rowNum, colNum, elem) {
		return _Utils_eq(
			...
			_List_fromArray(
				[
					elm$html$Html$Attributes$class('cursorCell')
				]),
			_List_fromArray(
				[
					A2(
			...
			_List_fromArray(
				[
					...
					A2(author$project$Main$CellSelected, rowNum, colNum))
				]),
			_List_fromArray(
				[
...
var author$project$Main$createBody = F2(
	function (point, rows) {
		return A2(
			elm$html$Html$tbody,
			_List_Nil,
...
var author$project$Main$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				elm$html$Html$Attributes$id('all')
			]),
		_List_fromArray(
			[
				A2(
...
var author$project$Main$docView = A2(
	...
	elm$html$Html$Lazy$lazy(author$project$Main$view),
	A2(
		elm$core$Basics$composeR,
		elm$core$List$singleton,
		elm$browser$Browser$Document('Smart Lotter')));
var author$project$Main$Model = function (data) {
	return {data: data};
};
...
var author$project$Main$exportCsv = function (loadedCsv) {
	var unwrappedCsv = A2(elm$core$List$cons, loadedCsv.csv.headers, loadedCsv.csv.records);
	var file = A2(
		elm$core$String$join,
...
var author$project$Main$moveCursor = F2(
	function (key, loadedCsv) {
		...
		var lastX = elm$core$List$length(loadedCsv.csv.headers) - 1;
		switch (key.$) {
			case 'Tab':
		...
		switch (msg.$) {
			...
			case 'CsvLoaded':
				...
				return _Utils_Tuple2(
					...
					author$project$Main$focusCursor(0));
			case 'CsvRemoved':
				return _Utils_Tuple2(
					_Utils_update(
						model,
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							data: A2(
								elm$core$Maybe$map,
								author$project$Main$newSelected(
									A2(author$project$Main$Point, rowNum, colNum)),
								model.data)
						}),
					author$project$Main$focusCursor(rowNum));
			case 'CellEdited':
				var newText = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							data: A2(
								elm$core$Maybe$map,
								author$project$Main$newEdit(newText),
								model.data)
						}),
					elm$core$Platform$Cmd$none);
			case 'FilenameEdited':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							data: A2(
								elm$core$Maybe$map,
								author$project$Main$newFilename(newText),
								model.data)
						}),
					elm$core$Platform$Cmd$none);
